...
class TestExtractLastPostExcerpt:
    def test_extracts_from_last_post(self):
        html = """
        <div class="pr-a">
            <div class="pr-j"><a href="/index.php?showuser=1">Adrien Nowak</a></div>
            <div class="postcolor">Adrien's post content here.</div>
        </div>
        <div class="pr-a">
            <div class="pr-j"><a href="/index.php?showuser=2">Mura Saiz</a></div>
            <div class="postcolor">Mura's actual response text.</div>
        </div>
        """
        excerpt = extract_last_post_excerpt(html)
        assert excerpt == "Mura's actual response text."

    def test_strips_jcink_quote_blocks(self):
        """Reply-with-quote inserts quotetop/quotemain at the top of a post.

        The excerpt must reflect what the poster wrote, not the quoted text
        from the previous poster.
        """
        html = """
        <div class="pr-a">
            <div class="pr-j"><a href="/index.php?showuser=1">Adrien Nowak</a></div>
            <div class="postcolor">Adrien wrote something original here.</div>
        </div>
        <div class="pr-a">
            <div class="pr-j"><a href="/index.php?showuser=2">Mura Saiz</a></div>
            <div class="postcolor">
                <div class="quotetop">QUOTE (Adrien Nowak @ Jan 15 2026, 08:30 PM)</div>
                <div class="quotemain">Adrien wrote something original here.</div>
                Mura thought for a moment before responding.
            </div>
        </div>
        """
        excerpt = extract_last_post_excerpt(html)
        assert "Mura thought" in excerpt
        assert "Adrien" not in excerpt
        assert "QUOTE" not in excerpt

    def test_strips_blockquote_elements(self):
        html = """
        <div class="pr-a">
            <div class="pr-j"><a href="/index.php?showuser=2">Mura Saiz</a></div>
            <div class="postcolor">
                <blockquote>Quoted text from another character's post.</blockquote>
                Mura's actual writing follows the quote.
            </div>
        </div>
        """
        excerpt = extract_last_post_excerpt(html)
        assert "Mura's actual writing" in excerpt
        assert "Quoted text" not in excerpt

    def test_strips_nested_quote_blocks(self):
        """Some posts quote multiple previous posts."""
        html = """
        <div class="pr-a">
            <div class="pr-j"><a href="/index.php?showuser=2">Mura Saiz</a></div>
            <div class="postcolor">
                <div class="quotetop">QUOTE (Adrien Nowak)</div>
                <div class="quotemain">First quoted passage.</div>
                <div class="quotetop">QUOTE (Another Character)</div>
                <div class="quotemain">Second quoted passage.</div>
                Mura responds to both characters here.
            </div>
        </div>
        """
        excerpt = extract_last_post_excerpt(html)
        assert "Mura responds" in excerpt
        assert "First quoted" not in excerpt
        assert "Second quoted" not in excerpt

    def test_returns_none_when_only_quotes(self):
        """If the post is entirely quoted text with no original content."""
        html = """
        <div class="pr-a">
            <div class="pr-j"><a href="/index.php?showuser=2">Mura Saiz</a></div>
            <div class="postcolor">
                <div class="quotetop">QUOTE</div>
                <div class="quotemain">Only quoted content, no original text.</div>
            </div>
        </div>
        """
        excerpt = extract_last_post_excerpt(html)
    ...
    def test_truncates_long_excerpt(self):
        ...
        html = f"""
        <div class="pr-a">
            <div class="pr-j">Poster</div>
            <div class="postcolor">{long_text}</div>
        </div>
        """
        excerpt = extract_last_post_excerpt(html)
        assert len(excerpt) <= 201  # 200 + ellipsis char
        assert excerpt.endswith("\u2026")

    def test_no_posts_returns_none(self):
        html = "<html><body>No posts here</body></html>"
        assert extract_last_post_excerpt(html) is None

    def test_no_postcolor_returns_none(self):
        html = """
        <div class="pr-a">
            <div class="pr-j">Poster</div>
        </div>
